...
class ControllerCatalogOctestimonial extends Controller {
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('catalog/octestimonial', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
    ...
    protected function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('catalog/octestimonial', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' / '
        );
        $data['insert'] = $this->url->link('catalog/octestimonial/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $data['delete'] = $this->url->link('catalog/octestimonial/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');

		foreach ($results as $result) {
			$action = array();
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('catalog/octestimonial/update', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $result['testimonial_id'])
				);
      		$data['testimonials'][] = array(
		...
		$pagination->url = HTTPS_SERVER . 'index.php?route=catalog/octestimonial&token=' . $this->session->data['token'] . $url . '&page={page}';
  	...
  	private function getForm() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('catalog/octestimonial', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' / '
   		);


		if (!isset($this->request->get['testimonial_id'])) {
			$data['action'] = $this->url->link('catalog/octestimonial/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/octestimonial/update', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $this->request->get['testimonial_id']);
		}
		
		$data['cancel'] = $this->url->link('catalog/octestimonial', 'token=' . $this->session->data['token'], 'SSL');
