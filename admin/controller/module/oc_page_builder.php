...
class ControllerModuleOcPageBuilder extends Controller
{
    private $error = array();

    public function index() {
        $this->load->language('module/oc_page_builder');
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('module/oc_page_builder', 'token=' . $this->session->data['token'], true)
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('module/oc_page_builder', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
            );
        }

        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('module/oc_page_builder', 'token=' . $this->session->data['token'], true);
        } else {
            $data['action'] = $this->url->link('module/oc_page_builder', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
        }
        ...
        foreach ($extensions as $code) {
            ...
            foreach ($modules as $module) {
                $module_data[] = array(
                    ...
                    'url'	=> $this->url->link('module/' . $code, 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], true)
                );
            }
            ...
            if ($this->config->has($code . '_status') || $module_data) {
                $data['extensions'][] = array(
                    ...
                    'code'   => $code,
                    'url'	 => $this->url->link('module/' . $code, 'token=' . $this->session->data['token'], true),
                    'module' => $module_data
                );
            }
        }
        ...
        $this->response->setOutput($this->load->view('module/ocpagebuilder/oc_page_builder', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'module/oc_page_builder')) {
